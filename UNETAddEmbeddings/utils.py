...
def reverse_diffusion(model, diffusion_steps, device=device, size=(32,32), z_dim = 32):
    step_size = 1.0 / diffusion_steps
    current_images = torch.randn(1, z_dim, size[0], size[1]).to(device)
    with torch.no_grad():
        model.eval()
...
def reverse_diffusion_cfg(model, diffusion_steps, category, cfg_scale, device=device, z_dim = 32,size=(32, 32)):
    step_size = 1.0 / diffusion_steps
    current_images = torch.randn(1, z_dim, size[0], size[1]).to(device)
    model.eval()
